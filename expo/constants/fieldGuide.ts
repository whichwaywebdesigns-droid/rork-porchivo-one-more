/**
 * Porchivo Field Guide content.
 *
 * Static, in-repo content for the in-app instruction manual ("The Porchivo
 * Field Guide"). Numbers/stats wrapped in **double asterisks** are rendered
 * bold + brand-orange to anchor key facts.
 */
...
/**
 * Who a section is most relevant to.
 * - `all`: every neighbor (shown to everyone).
 * - `homeowner`: people protecting their own deliveries (senders/recipients).
 * - `partner`: people who hold packages for neighbors and earn.
 */
export type GuideAudience = 'all' | 'homeowner' | 'partner';

export interface ManualSection {
  /** Stable id used as the AsyncStorage completion key. */
  id: string;
  /** Ionicons glyph name. */
  icon: string;
  title: string;
  teaser: string;
  /** Primary audience this section is written for. */
  audience: GuideAudience;
  blocks: ManualBlock[];
}

/** Short pill label for a role-specific section. */
export function audienceLabel(audience: GuideAudience): string | null {
  switch (audience) {
    case 'homeowner':
      return 'For senders';
    case 'partner':
