/**
 * Porchivo — UI translation strings.
 *
 * Hand-written translations for site chrome (header, footer, settings).
 * Keys map 1:1 across locales. Untranslated locales fall back to `en`.
 * Keep keys flat and namespaced by area (nav.*, footer.*, settings.*).
 */

const en = {
  "nav.howItWorks": "How It Works",
  "nav.features": "Features",
  "nav.porchPartners": "Porch Partners",
  "nav.pricing": "Pricing",
  "nav.help": "Help",
  "nav.getAppFree": "Get the app free",
  "nav.navigate": "Navigate",
  "nav.getAppFreeLong": "Get Porchivo free — iOS & Android",
  "nav.faqPrompt": "Have a question? Read the FAQ",

  "footer.product": "Product",
  "footer.company": "Company",
  "footer.legal": "Legal & Trust",
  "footer.developers": "Developers & AI",
  "footer.settings": "Settings",
  "footer.rights": "All rights reserved.",
  "footer.privacy": "Privacy",
  "footer.terms": "Terms",
  "footer.iosAndroid": "iOS & Android",
  "footer.freeToDownload": "Free to download",

  "settings.title": "Settings",
  "settings.subtitle": "Personalize how Porchivo's website works for you.",
  "settings.language.title": "Language",
  "settings.language.description":
    "Choose your preferred language. Your selection is saved on this device.",
  "settings.language.label": "Display language",
  "settings.language.search": "Search languages…",
  "settings.language.empty": "No language found.",
  "settings.language.translated": "Fully translated",
  "settings.language.beta": "Locale applied · text in English",
  "settings.language.current": "Current language",
  "settings.language.rtl": "Right-to-left",
  "settings.language.note":
    "Languages marked “Locale applied” set your region formatting and reading direction while we finish translating the full site.",
  "settings.preview.title": "Live preview",
  "settings.preview.sampleDate": "Today's date",
  "settings.preview.sampleNumber": "Packages protected",

  "nav.faq": "FAQ",

  "pwa.update.available": "A new version of Porchivo is available",
  "pwa.update.refresh": "Refresh",
  "pwa.update.dismiss": "Dismiss",
  "pwa.install.button": "Install Porchivo",
  "pwa.install.installedShort": "Installed",
  "pwa.install.bannerTitle": "Install Porchivo on this computer",
  "pwa.install.bannerBody": "Open Porchivo from your desktop or taskbar just like any other work app.",
  "pwa.install.successTitle": "Porchivo is installed",
  "pwa.install.installed":
    "Porchivo is installed. You can now open it from your desktop, taskbar, or Start menu.",
  "pwa.install.bannerDismiss": "Not now",
  "pwa.settings.desktop.title": "Desktop app",
  "pwa.settings.desktop.description":
    "Install Porchivo on Windows or Mac to open it from your desktop, taskbar, or Start menu — no browser tabs.",
  "pwa.settings.desktop.unavailable":
    "Installation isn't available in this browser. Open Porchivo in Chrome or Edge on your computer to install it.",
  "pwa.notifications.title": "Desktop notifications",
  "pwa.notifications.description":
    "Get a notification on this computer when something needs your attention. We only ask after you click enable.",
  "pwa.notifications.enable": "Enable notifications",
  "pwa.notifications.statusDefault": "Not enabled yet",
  "pwa.notifications.statusGranted": "Enabled — you'll hear from us when it matters",
  "pwa.notifications.statusDenied": "Blocked in your browser settings",
  "pwa.notifications.statusUnsupported": "Not supported on this device",
  "installPage.title": "Install Porchivo on Your Computer",
  "installPage.subtitle": "Open Porchivo from your desktop or taskbar just like any other work app.",
  "installPage.edge.title": "Install on Microsoft Edge",
  "installPage.chrome.title": "Install on Google Chrome",
  "installPage.edge.step1": "Open Porchivo in Microsoft Edge.",
  "installPage.edge.step2": "Click the install icon in the address bar, or open the three-dot menu.",
  "installPage.edge.step3": "Select Apps, then Install Porchivo.",
  "installPage.edge.step4":
    "Select desktop shortcut, taskbar pin, and Start menu options if Edge presents them.",
  "installPage.edge.step5": "Click Install.",
  "installPage.chrome.step1": "Open Porchivo in Google Chrome.",
  "installPage.chrome.step2": "Click the install icon in the address bar, or open the three-dot menu.",
  "installPage.chrome.step3": "Select Cast, save and share.",
  "installPage.chrome.step4": "Select Install page as app.",
  "installPage.chrome.step5": "Click Install.",
  "installPage.fallback":
    "If you do not see an install option, save Porchivo as a browser bookmark or ask your organization's IT administrator for assistance.",
  "installPage.open": "Open Porchivo",
  "installPage.returnToSignIn": "Return to Sign In",
  "installPage.recommended": "Recommended for your browser",
  "installPage.alreadyInstalled": "Porchivo is already installed on this device",
  "auth.sharedComputer": "Using a shared or front-desk computer? Sign out when you're done.",
  "portal.table.search": "Search…",
  "portal.table.filterAll": "All",
  "portal.table.prevPage": "Previous page",
  "portal.table.nextPage": "Next page",
  "portal.table.pageOf": "Page {{current}} of {{total}}",
  "portal.table.emptyTitle": "Nothing here yet",
  "portal.table.emptyBody": "New entries will appear here as your community uses Porchivo.",
  "portal.table.sortBy": "Sort by {{column}}",
  "portal.table.clearSearch": "Clear search",

  "landing.cta.register": "Register Your Community",

  "landing.hero.tagline": "Package Security & Resident Retention for Communities",
  "landing.hero.title1": "Know before it's",
  "landing.hero.title2": "too late.",
  "landing.hero.subtitle":
    "Real-time package risk scoring, instant alerts to residents and Porch Partners, and a neighbor-held delivery network — with no hardware or IT project required.",
  "landing.hero.subline":
    "Porchivo reduces management workload, surfaces community insights, and keeps residents satisfied — so they renew.",
  "landing.hero.ctaJoin": "Residents Join Free",
  "landing.hero.trust": "Built to work in 190+ countries worldwide",
  "landing.hero.stat1": "packages stolen in the US every year",
  "landing.hero.stat2": "of delivered packages are stolen from porches",
  "landing.hero.stat3": "to register a community and start protecting deliveries",
  "landing.hero.modelAlt": "3D model of a neighborhood protected by a glowing shield",

  "landing.how.title": "How Porchivo works for your community",
  "landing.how.sub":
    "No hardware. No IT project. Protect every resident's deliveries in four simple steps.",
  "landing.how.s1.title": "Register your community",
  "landing.how.s1.body":
    "~5 minute setup, $0 hardware, no IT project. No fragile 'let's loop in IT' project that somehow lasts until the sun burns out.",
  "landing.how.s2.title": "Residents join free",
  "landing.how.s2.body":
    "One invite code (think WLC726) and about a minute to join. Always free — no in-app purchases, no upsells, ever. Optionally, residents become Porch Partners: earn per secure handoff and build a reputation score. The board controls whether the network is on.",
  "landing.how.s3.title": "Real-time risk scoring",
  "landing.how.s3.body":
    "Every package is scored 0–100 before it lands, from three factors: delivery timing pattern, neighborhood theft activity, and community delivery history. Scores keep updating until handoff.",
  "landing.how.s4.title": "Alerts trigger action",
  "landing.how.s4.body":
    "When a package crosses the risk threshold, residents and nearby Porch Partners are notified instantly. Every handoff gets a full chain-of-custody log — timestamps, photos, identities — that managers can export.",
  "landing.how.more": "Read the full story",

  "landing.features.title":
    "Everything your community needs to protect deliveries and retain residents",
  "landing.features.sub":
    "Real-time scoring, alerts, Porch Partner handoffs, and manager insights — all in one app.",
  "landing.features.f1.title": "Real-Time Risk Scoring",
  "landing.features.f1.copy": "Every incoming package is scored for theft risk before it lands.",
  "landing.features.f1.alt": "Illustration of a package receiving a live theft-risk score",
  "landing.features.f2.title": "Porch Partner Network",
  "landing.features.f2.copy":
    "Join the Porch Partner network — safer deliveries, extra income, and community reputation.",
  "landing.features.f2.alt": "Illustration of Porch Partners handing off packages safely",
  "landing.features.f3.title": "Community Insights",
  "landing.features.f3.copy": "Managers see active risk zones, theft hotspots, and delivery congestion.",
  "landing.features.f3.alt": "Illustration of a manager's community risk map",
  "landing.features.badge": "Zero-touch deployment",
  "landing.features.setupTitle": "Five-Minute Community Setup",
  "landing.features.setupCopy": "No hardware, no IT project, no installation.",
  "landing.features.vaultAlt": "3D model of a secure package vault showing chain-of-custody",
  "landing.features.vaultFallback": "A secure package vault with chain-of-custody",

  "landing.pricing.title": "Residents join free. Communities subscribe from $99/mo.",
  "landing.pricing.sub":
    "HOAs and property managers pay one simple subscription. Residents get full access at no cost — no in-app purchases, no upsells.",
  "landing.pricing.t1.tagline": "For single communities getting started",
  "landing.pricing.t1.f1": "Real-time risk scoring for every package",
  "landing.pricing.t1.f2": "Instant alerts to residents & Porch Partners",
  "landing.pricing.t1.f3": "Porch Partner delivery network",
  "landing.pricing.t1.f4": "Full chain-of-custody for every handoff",
  "landing.pricing.t2.tagline": "For communities that want the full picture",
  "landing.pricing.t2.f1": "Everything in Essential",
  "landing.pricing.t2.f2": "Community insights — risk zones, theft hotspots, delivery congestion",
  "landing.pricing.t2.f3": "Manager dashboard with live community activity",
  "landing.pricing.t2.f4": "Priority support & guided onboarding",
  "landing.pricing.t3.tagline": "For portfolios and multi-property operators",
  "landing.pricing.t3.f1": "Everything in Professional",
  "landing.pricing.t3.f2": "Multi-property portfolio management",
  "landing.pricing.t3.f3": "Custom reporting & data export",
  "landing.pricing.t3.f4": "Dedicated success manager",
  "landing.pricing.popular": "Most popular",
  "landing.pricing.perMonth": "/mo",
  "landing.pricing.footnote": "Residents always join free.",

  "landing.faq.title": "Questions, answered",
  "landing.faq.q1": "Does Porchivo require cameras or hardware installed in our community?",
  "landing.faq.a1":
    "No. Porchivo is a software platform — there's no hardware to buy, no installation, and no IT project for your team. Residents use the phones they already have, and managers get a web dashboard. Most communities are fully registered and protecting deliveries in under five minutes.",
  "landing.faq.q2": "How does package risk scoring actually work?",
  "landing.faq.a2":
    "Every incoming delivery is scored in real time based on timing patterns, neighborhood theft activity, and your community's delivery history. Packages crossing a risk threshold trigger instant alerts to the resident and nearby Porch Partners — trusted neighbors who've opted in to receive and hold deliveries safely. Every handoff is logged with a full chain-of-custody record.",
  "landing.faq.q3": "What is a Porch Partner, and how do residents become one?",
  "landing.faq.a3":
    "Porch Partners are residents who opt in to receive packages for neighbors when risk is high or no one's home. Partners earn extra income per secure handoff and build a neighborhood reputation score. It's opt-in only — no resident is ever obligated, and your board controls whether the network is enabled for your community.",
  "landing.faq.q4": "How do our residents get started?",
  "landing.faq.a4":
    "Residents download the app and join with your community's invite code — it takes about a minute. They always join free: no in-app purchases, no upsells, no cost to residents ever. Your management team just distributes the invite code through your existing email or resident portal.",
  "landing.faq.q5": "What does the manager dashboard show?",
  "landing.faq.a5":
    "Managers see active risk zones, theft hotspots, delivery congestion patterns, and chain-of-custody records for every protected handoff — plus resident engagement and satisfaction signals that help you spot renewal risk before it becomes a resignation letter. All exportable for board reports.",

  "landing.final.title1": "Know before it's too late.",
  "landing.final.title2": "Register your community in two minutes.",

  "landing.videos.sectionAria": "Porchivo videos",
  "landing.videos.eyebrow": "From the channel",
  "landing.videos.title1": "See Porchivo",
  "landing.videos.title2": "in action",
  "landing.videos.play": "Play video: {{title}}",
  "landing.videos.prev": "Previous video",
  "landing.videos.next": "Next video",
  "landing.videos.goTo": "Go to video {{index}}",
  "landing.videos.v1.title": "Don't Have a Wonderful Day in the Neighborhood — For a Porch Pirate",
  "landing.videos.v1.desc":
    "Our signature short film: a porch pirate's perfect day takes a turn when the whole block is watching. See what happens when neighbors stop being easy targets.",
  "landing.videos.v2.title": "The App That's Making HOA Mailrooms Chaos-Free",
  "landing.videos.v2.desc":
    "A 60-second look at how communities replace overflowing mailroom chaos with tracked, claimed, and protected package hand-offs.",
  "landing.videos.v3.title": "Neighbors Protecting Neighbors — Stopping Porch Pirates for Good",
  "landing.videos.v3.desc":
    "The idea behind Porchivo: real neighbors, real accountability, and a delivery network that makes your porch the safest spot on the street.",
  "landing.videos.watchOnYoutube": "Watch on YouTube",
  "landing.returning.manager": "Welcome back — open your manager portal",
  "landing.returning.resident": "Welcome back — open your dashboard",
  "landing.returning.dismiss": "Dismiss",
};

/** Translation keys with string values (en is the canonical key set). */
export type TranslationKeys = Record<keyof typeof en, string>;

const es: Partial<TranslationKeys> = {
  "nav.howItWorks": "Cómo Funciona",
  "nav.features": "Funciones",
  "nav.porchPartners": "Vecinos de Confianza",
  "nav.pricing": "Precios",
  "nav.help": "Ayuda",
  "nav.getAppFree": "Descarga la app gratis",
  "nav.navigate": "Navegar",
  "nav.getAppFreeLong": "Descarga Porchivo gratis — iOS y Android",
  "nav.faqPrompt": "¿Tienes una pregunta? Lee las preguntas frecuentes",
  "footer.product": "Producto",
  "footer.company": "Empresa",
  "footer.legal": "Legal y Confianza",
  "footer.developers": "Desarrolladores e IA",
  "footer.settings": "Ajustes",
  "footer.rights": "Todos los derechos reservados.",
  "footer.privacy": "Privacidad",
  "footer.terms": "Términos",
  "footer.iosAndroid": "iOS y Android",
  "footer.freeToDownload": "Descarga gratuita",
  "settings.title": "Ajustes",
  "settings.subtitle": "Personaliza cómo funciona el sitio web de Porchivo para ti.",
  "settings.language.title": "Idioma",
  "settings.language.description":
    "Elige tu idioma preferido. Tu selección se guarda en este dispositivo.",
  "settings.language.label": "Idioma de visualización",
  "settings.language.search": "Buscar idiomas…",
  "settings.language.empty": "No se encontró ningún idioma.",
  "settings.language.translated": "Totalmente traducido",
  "settings.language.beta": "Configuración regional · texto en inglés",
  "settings.language.current": "Idioma actual",
  "settings.language.rtl": "De derecha a izquierda",
  "settings.language.note":
    "Los idiomas marcados como “Configuración regional” ajustan el formato de tu región y la dirección de lectura mientras terminamos de traducir todo el sitio.",
  "settings.preview.title": "Vista previa en vivo",
  "settings.preview.sampleDate": "Fecha de hoy",
  "settings.preview.sampleNumber": "Paquetes protegidos",

  "nav.faq": "Preguntas",

  "pwa.update.available": "Hay una nueva versión de Porchivo disponible",
  "pwa.update.refresh": "Actualizar",
  "pwa.update.dismiss": "Cerrar",
  "pwa.install.button": "Instalar Porchivo",
  "pwa.install.installedShort": "Instalado",
  "pwa.install.bannerTitle": "Instala Porchivo en esta computadora",
  "pwa.install.bannerBody": "Abre Porchivo desde tu escritorio o barra de tareas como cualquier otra aplicación de trabajo.",
  "pwa.install.successTitle": "Porchivo está instalado",
  "pwa.install.installed":
    "Porchivo está instalado. Ya puedes abrirlo desde tu escritorio, barra de tareas o menú de inicio.",
  "pwa.install.bannerDismiss": "Ahora no",
  "pwa.settings.desktop.title": "Aplicación de escritorio",
  "pwa.settings.desktop.description":
    "Instala Porchivo en Windows o Mac para abrirlo desde tu escritorio, barra de tareas o menú de inicio — sin pestañas del navegador.",
  "pwa.settings.desktop.unavailable":
    "La instalación no está disponible en este navegador. Abre Porchivo en Chrome o Edge en tu computadora para instalarlo.",
  "pwa.notifications.title": "Notificaciones de escritorio",
  "pwa.notifications.description":
    "Recibe una notificación en esta computadora cuando algo requiera tu atención. Solo te lo pediremos después de que hagas clic en activar.",
  "pwa.notifications.enable": "Activar notificaciones",
  "pwa.notifications.statusDefault": "Aún no activadas",
  "pwa.notifications.statusGranted": "Activadas — te avisaremos cuando importe",
  "pwa.notifications.statusDenied": "Bloqueadas en la configuración de tu navegador",
  "pwa.notifications.statusUnsupported": "No compatibles con este dispositivo",
  "installPage.title": "Instala Porchivo en tu computadora",
  "installPage.subtitle": "Abre Porchivo desde tu escritorio o barra de tareas como cualquier otra aplicación de trabajo.",
  "installPage.edge.title": "Instalar en Microsoft Edge",
  "installPage.chrome.title": "Instalar en Google Chrome",
  "installPage.edge.step1": "Abre Porchivo en Microsoft Edge.",
  "installPage.edge.step2": "Haz clic en el ícono de instalación en la barra de direcciones, o abre el menú de tres puntos.",
  "installPage.edge.step3": "Selecciona Aplicaciones y luego Instalar Porchivo.",
  "installPage.edge.step4":
    "Selecciona las opciones de acceso directo en el escritorio, anclaje en la barra de tareas y menú de inicio si Edge las muestra.",
  "installPage.edge.step5": "Haz clic en Instalar.",
  "installPage.chrome.step1": "Abre Porchivo en Google Chrome.",
  "installPage.chrome.step2": "Haz clic en el ícono de instalación en la barra de direcciones, o abre el menú de tres puntos.",
  "installPage.chrome.step3": "Selecciona Enviar, guardar y compartir.",
  "installPage.chrome.step4": "Selecciona Instalar página como aplicación.",
  "installPage.chrome.step5": "Haz clic en Instalar.",
  "installPage.fallback":
    "Si no ves una opción de instalación, guarda Porchivo como marcador en tu navegador o pide ayuda al administrador de TI de tu organización.",
  "installPage.open": "Abrir Porchivo",
  "installPage.returnToSignIn": "Volver a iniciar sesión",
  "installPage.recommended": "Recomendado para tu navegador",
  "installPage.alreadyInstalled": "Porchivo ya está instalado en este dispositivo",
  "auth.sharedComputer": "¿Usas una computadora compartida o de recepción? Cierra sesión al terminar.",
  "portal.table.search": "Buscar…",
  "portal.table.filterAll": "Todas",
  "portal.table.prevPage": "Página anterior",
  "portal.table.nextPage": "Página siguiente",
  "portal.table.pageOf": "Página {{current}} de {{total}}",
  "portal.table.emptyTitle": "Nada por aquí todavía",
  "portal.table.emptyBody": "Las nuevas entradas aparecerán aquí a medida que tu comunidad use Porchivo.",
  "portal.table.sortBy": "Ordenar por {{column}}",
  "portal.table.clearSearch": "Limpiar búsqueda",

  "landing.cta.register": "Registra tu comunidad",

  "landing.hero.tagline": "Seguridad de paquetes y retención de residentes para comunidades",
  "landing.hero.title1": "Entérate antes de que",
  "landing.hero.title2": "sea demasiado tarde.",
  "landing.hero.subtitle":
    "Puntaje de riesgo de paquetes en tiempo real, alertas instantáneas a residentes y Porch Partners, y una red de entregas sostenida por vecinos — sin hardware ni proyectos de TI.",
  "landing.hero.subline":
    "Porchivo reduce la carga operativa, revela información de tu comunidad y mantiene satisfechos a los residentes — para que renueven.",
  "landing.hero.ctaJoin": "Únete gratis como residente",
  "landing.hero.trust": "Diseñado para funcionar en más de 190 países",
  "landing.hero.stat1": "paquetes robados al año en EE. UU.",
  "landing.hero.stat2": "de los paquetes entregados se roban del porche",
  "landing.hero.stat3": "para registrar una comunidad y empezar a proteger entregas",
  "landing.hero.modelAlt": "Modelo 3D de un vecindario protegido por un escudo luminoso",

  "landing.how.title": "Cómo funciona Porchivo en tu comunidad",
  "landing.how.sub":
    "Sin hardware. Sin proyectos de TI. Protege las entregas de cada residente en cuatro pasos sencillos.",
  "landing.how.s1.title": "Registra tu comunidad",
  "landing.how.s1.body":
    "Configuración de ~5 minutos, $0 en hardware, sin proyecto de TI. Nada del frágil 'involucremos a TI' que dura hasta que el sol se apague.",
  "landing.how.s2.title": "Los residentes se unen gratis",
  "landing.how.s2.body":
    "Un código de invitación (tipo WLC726) y alrededor de un minuto para unirse. Siempre gratis — sin compras dentro de la app, sin ventas adicionales, nunca. Opcional: los residentes pueden ser Porch Partners, ganar por cada entrega segura y construir un puntaje de reputación. La mesa directiva decide si la red está activada.",
  "landing.how.s3.title": "Puntaje de riesgo en tiempo real",
  "landing.how.s3.body":
    "Cada paquete recibe un puntaje de 0 a 100 antes de llegar, según tres factores: patrón de horarios de entrega, actividad de robo en el vecindario e historial de entregas de la comunidad. El puntaje se actualiza hasta la entrega.",
  "landing.how.s4.title": "Las alertas activan acciones",
  "landing.how.s4.body":
    "Cuando un paquete cruza el umbral de riesgo, residentes y Porch Partners cercanos reciben una notificación al instante. Cada entrega queda con un registro completo de cadena de custodia — horarios, fotos, identidades — que los administradores pueden exportar.",
  "landing.how.more": "Lee la historia completa",

  "landing.features.title":
    "Todo lo que tu comunidad necesita para proteger entregas y retener residentes",
  "landing.features.sub":
    "Puntajes en tiempo real, alertas, entregas de Porch Partners e información para administradores — todo en una sola app.",
  "landing.features.f1.title": "Puntaje de riesgo en tiempo real",
  "landing.features.f1.copy": "Cada paquete entrante se evalúa por riesgo de robo antes de llegar.",
  "landing.features.f1.alt": "Ilustración de un paquete recibiendo un puntaje de riesgo de robo en vivo",
  "landing.features.f2.title": "Red de Porch Partners",
  "landing.features.f2.copy":
    "Únete a la red de Porch Partners — entregas más seguras, ingresos extra y reputación comunitaria.",
  "landing.features.f2.alt": "Ilustración de Porch Partners entregando paquetes de forma segura",
  "landing.features.f3.title": "Perspectivas de la comunidad",
  "landing.features.f3.copy":
    "Los administradores ven zonas de riesgo activas, puntos críticos de robo y congestión de entregas.",
  "landing.features.f3.alt": "Ilustración del mapa de riesgo comunitario de un administrador",
  "landing.features.badge": "Implementación sin fricción",
  "landing.features.setupTitle": "Configura tu comunidad en cinco minutos",
  "landing.features.setupCopy": "Sin hardware, sin proyectos de TI, sin instalación.",
  "landing.features.vaultAlt": "Modelo 3D de una bóveda segura de paquetes con cadena de custodia",
  "landing.features.vaultFallback": "Una bóveda segura de paquetes con cadena de custodia",

  "landing.pricing.title": "Los residentes se unen gratis. Las comunidades tienen planes desde $99/mes.",
  "landing.pricing.sub":
    "Las asociaciones y administradores de propiedades pagan una sola suscripción. Los residentes tienen acceso completo sin costo — sin compras dentro de la app ni ventas adicionales.",
  "landing.pricing.t1.tagline": "Para comunidades individuales que están comenzando",
  "landing.pricing.t1.f1": "Puntaje de riesgo en tiempo real para cada paquete",
  "landing.pricing.t1.f2": "Alertas instantáneas a residentes y Porch Partners",
  "landing.pricing.t1.f3": "Red de entregas de Porch Partners",
  "landing.pricing.t1.f4": "Cadena de custodia completa en cada entrega",
  "landing.pricing.t2.tagline": "Para comunidades que quieren ver el panorama completo",
  "landing.pricing.t2.f1": "Todo lo de Essential",
  "landing.pricing.t2.f2":
    "Perspectivas comunitarias: zonas de riesgo, puntos críticos de robo y congestión de entregas",
  "landing.pricing.t2.f3": "Panel de administración con actividad comunitaria en vivo",
  "landing.pricing.t2.f4": "Soporte prioritario e incorporación guiada",
  "landing.pricing.t3.tagline": "Para portafolios y operadores de múltiples propiedades",
  "landing.pricing.t3.f1": "Todo lo de Professional",
  "landing.pricing.t3.f2": "Administración de portafolio de múltiples propiedades",
  "landing.pricing.t3.f3": "Reportes personalizados y exportación de datos",
  "landing.pricing.t3.f4": "Gerente de éxito dedicado",
  "landing.pricing.popular": "El más popular",
  "landing.pricing.perMonth": "/mes",
  "landing.pricing.footnote": "Los residentes siempre se unen gratis.",

  "landing.faq.title": "Preguntas, respondidas",
  "landing.faq.q1": "¿Porchivo requiere cámaras o hardware instalado en nuestra comunidad?",
  "landing.faq.a1":
    "No. Porchivo es una plataforma de software — no hay hardware que comprar, ni instalación, ni proyectos de TI para tu equipo. Los residentes usan los teléfonos que ya tienen y los administradores obtienen un panel web. La mayoría de las comunidades están registradas y protegiendo entregas en menos de cinco minutos.",
  "landing.faq.q2": "¿Cómo funciona realmente el puntaje de riesgo de paquetes?",
  "landing.faq.a2":
    "Cada entrega entrante se evalúa en tiempo real según patrones de horarios, actividad de robo en el vecindario y el historial de entregas de tu comunidad. Los paquetes que cruzan un umbral de riesgo activan alertas instantáneas al residente y a los Porch Partners cercanos — vecinos de confianza que optaron por recibir y resguardar entregas de forma segura. Cada entrega queda registrada con una cadena de custodia completa.",
  "landing.faq.q3": "¿Qué es un Porch Partner y cómo se convierten los residentes en uno?",
  "landing.faq.a3":
    "Los Porch Partners son residentes que optan por recibir paquetes de sus vecinos cuando el riesgo es alto o no hay nadie en casa. Los partners ganan ingresos extra por cada entrega segura y construyen un puntaje de reputación en el vecindario. Es completamente voluntario — ningún residente está obligado, y tu mesa directiva controla si la red está activada para tu comunidad.",
  "landing.faq.q4": "¿Cómo empiezan nuestros residentes?",
  "landing.faq.a4":
    "Los residentes descargan la app y se unen con el código de invitación de tu comunidad — toma alrededor de un minuto. Siempre se unen gratis: sin compras dentro de la app, sin ventas adicionales y sin costo para los residentes. Tu equipo de administración solo distribuye el código de invitación por el correo o portal de residentes que ya usan.",
  "landing.faq.q5": "¿Qué muestra el panel de administración?",
  "landing.faq.a5":
    "Los administradores ven zonas de riesgo activas, puntos críticos de robo, patrones de congestión de entregas y registros de cadena de custodia de cada entrega protegida — además de señales de participación y satisfacción de los residentes que te ayudan a detectar el riesgo de no renovación a tiempo. Todo es exportable para los reportes de la mesa directiva.",

  "landing.final.title1": "Entérate antes de que sea demasiado tarde.",
  "landing.final.title2": "Registra tu comunidad en dos minutos.",

  "landing.videos.sectionAria": "Videos de Porchivo",
  "landing.videos.eyebrow": "Del canal",
  "landing.videos.title1": "Mira Porchivo",
  "landing.videos.title2": "en acción",
  "landing.videos.play": "Reproducir video: {{title}}",
  "landing.videos.prev": "Video anterior",
  "landing.videos.next": "Video siguiente",
  "landing.videos.goTo": "Ir al video {{index}}",
  "landing.videos.v1.title": "No tengas un maravilloso día en el vecindario — si eres pirata de porches",
  "landing.videos.v1.desc":
    "Nuestro cortometraje: el día perfecto de un pirata de porches da un giro cuando todo el vecindario está atento. Descubre qué pasa cuando los vecinos dejan de ser objetivos fáciles.",
  "landing.videos.v2.title": "La app que está eliminando el caos en los cuartos de correo",
  "landing.videos.v2.desc":
    "Un vistazo de 60 segundos a cómo las comunidades reemplazan el caos de los cuartos de paquetes con entregas rastreadas, reclamadas y protegidas.",
  "landing.videos.v3.title": "Vecinos protegiendo vecinos — acabando con los piratas de porches para siempre",
  "landing.videos.v3.desc":
    "La idea detrás de Porchivo: vecinos reales, responsabilidad real y una red de entregas que hace de tu porche el lugar más seguro de la calle.",
  "landing.videos.watchOnYoutube": "Ver en YouTube",
  "landing.returning.manager": "Bienvenido de nuevo — abre tu portal de administración",
  "landing.returning.resident": "Bienvenido de nuevo — abre tu panel",
  "landing.returning.dismiss": "Cerrar",
};

const fr: Partial<TranslationKeys> = {
  "nav.howItWorks": "Comment ça marche",
  "nav.features": "Fonctionnalités",
  "nav.porchPartners": "Voisins de Confiance",
  "nav.pricing": "Tarifs",
  "nav.help": "Aide",
  "nav.getAppFree": "Télécharger l'app gratuitement",
  "nav.navigate": "Naviguer",
  "nav.getAppFreeLong": "Téléchargez Porchivo gratuitement — iOS et Android",
  "nav.faqPrompt": "Une question ? Consultez la FAQ",
  "footer.product": "Produit",
  "footer.company": "Entreprise",
  "footer.legal": "Mentions légales et confiance",
  "footer.developers": "Développeurs et IA",
  "footer.settings": "Paramètres",
  "footer.rights": "Tous droits réservés.",
  "footer.privacy": "Confidentialité",
  "footer.terms": "Conditions",
  "footer.iosAndroid": "iOS et Android",
  "footer.freeToDownload": "Téléchargement gratuit",
  "settings.title": "Paramètres",
  "settings.subtitle": "Personnalisez le fonctionnement du site Porchivo pour vous.",
  "settings.language.title": "Langue",
  "settings.language.description":
    "Choisissez votre langue préférée. Votre choix est enregistré sur cet appareil.",
  "settings.language.label": "Langue d'affichage",
  "settings.language.search": "Rechercher des langues…",
  "settings.language.empty": "Aucune langue trouvée.",
  "settings.language.translated": "Entièrement traduit",
  "settings.language.beta": "Paramètres régionaux · texte en anglais",
  "settings.language.current": "Langue actuelle",
  "settings.language.rtl": "De droite à gauche",
  "settings.language.note":
    "Les langues marquées « Paramètres régionaux » définissent le format régional et le sens de lecture pendant que nous terminons la traduction complète du site.",
  "settings.preview.title": "Aperçu en direct",
  "settings.preview.sampleDate": "Date du jour",
  "settings.preview.sampleNumber": "Colis protégés",
};

const de: Partial<TranslationKeys> = {
  "nav.howItWorks": "So funktioniert's",
  "nav.features": "Funktionen",
  "nav.porchPartners": "Vertrauensnachbarn",
  "nav.pricing": "Preise",
  "nav.help": "Hilfe",
  "nav.getAppFree": "App kostenlos holen",
  "nav.navigate": "Navigieren",
  "nav.getAppFreeLong": "Porchivo kostenlos holen — iOS & Android",
  "nav.faqPrompt": "Eine Frage? Lies die FAQ",
  "footer.product": "Produkt",
  "footer.company": "Unternehmen",
  "footer.legal": "Recht & Vertrauen",
  "footer.developers": "Entwickler & KI",
  "footer.settings": "Einstellungen",
  "footer.rights": "Alle Rechte vorbehalten.",
  "footer.privacy": "Datenschutz",
  "footer.terms": "AGB",
  "footer.iosAndroid": "iOS & Android",
  "footer.freeToDownload": "Kostenloser Download",
  "settings.title": "Einstellungen",
  "settings.subtitle": "Passe an, wie die Porchivo-Website für dich funktioniert.",
  "settings.language.title": "Sprache",
  "settings.language.description":
    "Wähle deine bevorzugte Sprache. Deine Auswahl wird auf diesem Gerät gespeichert.",
  "settings.language.label": "Anzeigesprache",
  "settings.language.search": "Sprachen suchen…",
  "settings.language.empty": "Keine Sprache gefunden.",
  "settings.language.translated": "Vollständig übersetzt",
  "settings.language.beta": "Gebietsschema aktiv · Text auf Englisch",
  "settings.language.current": "Aktuelle Sprache",
  "settings.language.rtl": "Von rechts nach links",
  "settings.language.note":
    "Mit „Gebietsschema aktiv“ markierte Sprachen setzen dein Regionsformat und die Leserichtung, während wir die vollständige Übersetzung fertigstellen.",
  "settings.preview.title": "Live-Vorschau",
  "settings.preview.sampleDate": "Heutiges Datum",
  "settings.preview.sampleNumber": "Geschützte Pakete",
};

const pt: Partial<TranslationKeys> = {
  "nav.howItWorks": "Como Funciona",
  "nav.features": "Recursos",
  "nav.porchPartners": "Vizinhos de Confiança",
  "nav.pricing": "Preços",
  "nav.help": "Ajuda",
  "nav.getAppFree": "Baixar o app grátis",
  "nav.navigate": "Navegar",
  "nav.getAppFreeLong": "Baixe o Porchivo grátis — iOS e Android",
  "nav.faqPrompt": "Tem uma pergunta? Leia o FAQ",
  "footer.product": "Produto",
  "footer.company": "Empresa",
  "footer.legal": "Jurídico e Confiança",
  "footer.developers": "Desenvolvedores e IA",
  "footer.settings": "Configurações",
  "footer.rights": "Todos os direitos reservados.",
  "footer.privacy": "Privacidade",
  "footer.terms": "Termos",
  "footer.iosAndroid": "iOS e Android",
  "footer.freeToDownload": "Download gratuito",
  "settings.title": "Configurações",
  "settings.subtitle": "Personalize como o site da Porchivo funciona para você.",
  "settings.language.title": "Idioma",
  "settings.language.description":
    "Escolha seu idioma preferido. Sua seleção é salva neste dispositivo.",
  "settings.language.label": "Idioma de exibição",
  "settings.language.search": "Pesquisar idiomas…",
  "settings.language.empty": "Nenhum idioma encontrado.",
  "settings.language.translated": "Totalmente traduzido",
  "settings.language.beta": "Formato regional · texto em inglês",
  "settings.language.current": "Idioma atual",
  "settings.language.rtl": "Da direita para a esquerda",
  "settings.language.note":
    "Idiomas marcados como “Formato regional” definem o formato da sua região e o sentido de leitura enquanto terminamos de traduzir todo o site.",
  "settings.preview.title": "Pré-visualização ao vivo",
  "settings.preview.sampleDate": "Data de hoje",
  "settings.preview.sampleNumber": "Pacotes protegidos",
};

const it: Partial<TranslationKeys> = {
  "nav.howItWorks": "Come Funziona",
  "nav.features": "Funzionalità",
  "nav.porchPartners": "Vicini di Fiducia",
  "nav.pricing": "Prezzi",
  "nav.help": "Aiuto",
  "nav.getAppFree": "Scarica l'app gratis",
  "nav.navigate": "Naviga",
  "nav.getAppFreeLong": "Scarica Porchivo gratis — iOS e Android",
  "nav.faqPrompt": "Hai una domanda? Leggi le FAQ",
  "footer.product": "Prodotto",
  "footer.company": "Azienda",
  "footer.legal": "Legale e Fiducia",
  "footer.developers": "Sviluppatori e IA",
  "footer.settings": "Impostazioni",
  "footer.rights": "Tutti i diritti riservati.",
  "footer.privacy": "Privacy",
  "footer.terms": "Termini",
  "footer.iosAndroid": "iOS e Android",
  "footer.freeToDownload": "Download gratuito",
  "settings.title": "Impostazioni",
  "settings.subtitle": "Personalizza il funzionamento del sito Porchivo per te.",
  "settings.language.title": "Lingua",
  "settings.language.description":
    "Scegli la tua lingua preferita. La tua selezione viene salvata su questo dispositivo.",
  "settings.language.label": "Lingua di visualizzazione",
  "settings.language.search": "Cerca lingue…",
  "settings.language.empty": "Nessuna lingua trovata.",
  "settings.language.translated": "Completamente tradotto",
  "settings.language.beta": "Formato regionale · testo in inglese",
  "settings.language.current": "Lingua attuale",
  "settings.language.rtl": "Da destra a sinistra",
  "settings.language.note":
    "Le lingue contrassegnate come “Formato regionale” impostano il formato della tua regione e la direzione di lettura mentre completiamo la traduzione dell'intero sito.",
  "settings.preview.title": "Anteprima in tempo reale",
  "settings.preview.sampleDate": "Data di oggi",
  "settings.preview.sampleNumber": "Pacchi protetti",
};

const ru: Partial<TranslationKeys> = {
  "nav.howItWorks": "Как это работает",
  "nav.features": "Возможности",
  "nav.porchPartners": "Надёжные соседи",
  "nav.pricing": "Цены",
  "nav.help": "Помощь",
  "nav.getAppFree": "Скачать приложение бесплатно",
  "nav.navigate": "Навигация",
  "nav.getAppFreeLong": "Скачайте Porchivo бесплатно — iOS и Android",
  "nav.faqPrompt": "Есть вопрос? Прочитайте FAQ",
  "footer.product": "Продукт",
  "footer.company": "Компания",
  "footer.legal": "Право и доверие",
  "footer.developers": "Разработчики и ИИ",
  "footer.settings": "Настройки",
  "footer.rights": "Все права защищены.",
  "footer.privacy": "Конфиденциальность",
  "footer.terms": "Условия",
  "footer.iosAndroid": "iOS и Android",
  "footer.freeToDownload": "Бесплатная загрузка",
  "settings.title": "Настройки",
  "settings.subtitle": "Настройте, как работает сайт Porchivo для вас.",
  "settings.language.title": "Язык",
  "settings.language.description":
    "Выберите предпочитаемый язык. Ваш выбор сохраняется на этом устройстве.",
  "settings.language.label": "Язык интерфейса",
  "settings.language.search": "Поиск языков…",
  "settings.language.empty": "Язык не найден.",
  "settings.language.translated": "Полностью переведено",
  "settings.language.beta": "Региональный формат · текст на английском",
  "settings.language.current": "Текущий язык",
  "settings.language.rtl": "Справа налево",
  "settings.language.note":
    "Языки с пометкой «Региональный формат» задают формат вашего региона и направление чтения, пока мы завершаем перевод всего сайта.",
  "settings.preview.title": "Живой предпросмотр",
  "settings.preview.sampleDate": "Сегодняшняя дата",
  "settings.preview.sampleNumber": "Защищённые посылки",
};

const zh: Partial<TranslationKeys> = {
  "nav.howItWorks": "工作原理",
  "nav.features": "功能",
  "nav.porchPartners": "门廊伙伴",
  "nav.pricing": "价格",
  "nav.help": "帮助",
  "nav.getAppFree": "免费获取应用",
  "nav.navigate": "导航",
  "nav.getAppFreeLong": "免费下载 Porchivo — iOS 和 Android",
  "nav.faqPrompt": "有问题吗？阅读常见问题",
  "footer.product": "产品",
  "footer.company": "公司",
  "footer.legal": "法律与信任",
  "footer.developers": "开发者与 AI",
  "footer.settings": "设置",
  "footer.rights": "保留所有权利。",
  "footer.privacy": "隐私",
  "footer.terms": "条款",
  "footer.iosAndroid": "iOS 和 Android",
  "footer.freeToDownload": "免费下载",
  "settings.title": "设置",
  "settings.subtitle": "个性化 Porchivo 网站的使用方式。",
  "settings.language.title": "语言",
  "settings.language.description": "选择您的首选语言。您的选择会保存在此设备上。",
  "settings.language.label": "显示语言",
  "settings.language.search": "搜索语言…",
  "settings.language.empty": "未找到语言。",
  "settings.language.translated": "完整翻译",
  "settings.language.beta": "已应用区域设置 · 文本为英文",
  "settings.language.current": "当前语言",
  "settings.language.rtl": "从右到左",
  "settings.language.note":
    "标记为“已应用区域设置”的语言会设置您的区域格式和阅读方向，同时我们正在完成整个网站的翻译。",
  "settings.preview.title": "实时预览",
  "settings.preview.sampleDate": "今天的日期",
  "settings.preview.sampleNumber": "已保护的包裹",
};

const ja: Partial<TranslationKeys> = {
  "nav.howItWorks": "使い方",
  "nav.features": "機能",
  "nav.porchPartners": "ポーチパートナー",
  "nav.pricing": "料金",
  "nav.help": "ヘルプ",
  "nav.getAppFree": "アプリを無料で入手",
  "nav.navigate": "ナビゲート",
  "nav.getAppFreeLong": "Porchivo を無料で入手 — iOS と Android",
  "nav.faqPrompt": "質問がありますか？FAQ をご覧ください",
  "footer.product": "製品",
  "footer.company": "会社",
  "footer.legal": "法的事項と信頼",
  "footer.developers": "開発者と AI",
  "footer.settings": "設定",
  "footer.rights": "全著作権所有。",
  "footer.privacy": "プライバシー",
  "footer.terms": "利用規約",
  "footer.iosAndroid": "iOS と Android",
  "footer.freeToDownload": "無料ダウンロード",
  "settings.title": "設定",
  "settings.subtitle": "Porchivo のウェブサイトの動作をカスタマイズします。",
  "settings.language.title": "言語",
  "settings.language.description": "ご希望の言語を選択してください。選択内容はこの端末に保存されます。",
  "settings.language.label": "表示言語",
  "settings.language.search": "言語を検索…",
  "settings.language.empty": "言語が見つかりません。",
  "settings.language.translated": "完全翻訳済み",
  "settings.language.beta": "ロケール適用 · テキストは英語",
  "settings.language.current": "現在の言語",
  "settings.language.rtl": "右から左",
  "settings.language.note":
    "「ロケール適用」と表示された言語は、サイト全体の翻訳が完了するまで、地域の書式と読む方向を設定します。",
  "settings.preview.title": "ライブプレビュー",
  "settings.preview.sampleDate": "今日の日付",
  "settings.preview.sampleNumber": "保護された荷物",
};

const ko: Partial<TranslationKeys> = {
  "nav.howItWorks": "이용 방법",
  "nav.features": "기능",
  "nav.porchPartners": "포치 파트너",
  "nav.pricing": "요금제",
  "nav.help": "도움말",
  "nav.getAppFree": "앱 무료로 받기",
  "nav.navigate": "탐색",
  "nav.getAppFreeLong": "Porchivo 무료로 받기 — iOS 및 Android",
  "nav.faqPrompt": "궁금한 점이 있나요? FAQ를 읽어보세요",
  "footer.product": "제품",
  "footer.company": "회사",
  "footer.legal": "법률 및 신뢰",
  "footer.developers": "개발자 및 AI",
  "footer.settings": "설정",
  "footer.rights": "모든 권리 보유.",
  "footer.privacy": "개인정보",
  "footer.terms": "약관",
  "footer.iosAndroid": "iOS 및 Android",
  "footer.freeToDownload": "무료 다운로드",
  "settings.title": "설정",
  "settings.subtitle": "Porchivo 웹사이트의 작동 방식을 맞춤 설정하세요.",
  "settings.language.title": "언어",
  "settings.language.description": "원하는 언어를 선택하세요. 선택 항목은 이 기기에 저장됩니다.",
  "settings.language.label": "표시 언어",
  "settings.language.search": "언어 검색…",
  "settings.language.empty": "언어를 찾을 수 없습니다.",
  "settings.language.translated": "완전 번역됨",
  "settings.language.beta": "로케일 적용됨 · 텍스트는 영어",
  "settings.language.current": "현재 언어",
  "settings.language.rtl": "오른쪽에서 왼쪽",
  "settings.language.note":
    "“로케일 적용됨”으로 표시된 언어는 전체 사이트 번역을 완료하는 동안 지역 형식과 읽기 방향을 설정합니다.",
  "settings.preview.title": "실시간 미리보기",
  "settings.preview.sampleDate": "오늘 날짜",
  "settings.preview.sampleNumber": "보호된 택배",
};

const hi: Partial<TranslationKeys> = {
  "nav.howItWorks": "यह कैसे काम करता है",
  "nav.features": "विशेषताएँ",
  "nav.porchPartners": "भरोसेमंद पड़ोसी",
  "nav.pricing": "मूल्य निर्धारण",
  "nav.help": "सहायता",
  "nav.getAppFree": "ऐप मुफ़्त पाएं",
  "nav.navigate": "नेविगेट करें",
  "nav.getAppFreeLong": "Porchivo मुफ़्त पाएं — iOS और Android",
  "nav.faqPrompt": "कोई प्रश्न है? FAQ पढ़ें",
  "footer.product": "उत्पाद",
  "footer.company": "कंपनी",
  "footer.legal": "कानूनी और भरोसा",
  "footer.developers": "डेवलपर्स और AI",
  "footer.settings": "सेटिंग्स",
  "footer.rights": "सर्वाधिकार सुरक्षित।",
  "footer.privacy": "गोपनीयता",
  "footer.terms": "शर्तें",
  "footer.iosAndroid": "iOS और Android",
  "footer.freeToDownload": "मुफ़्त डाउनलोड",
  "settings.title": "सेटिंग्स",
  "settings.subtitle": "Porchivo की वेबसाइट आपके लिए कैसे काम करती है, इसे निजीकृत करें।",
  "settings.language.title": "भाषा",
  "settings.language.description": "अपनी पसंदीदा भाषा चुनें। आपका चयन इस डिवाइस पर सहेजा जाता है।",
  "settings.language.label": "प्रदर्शन भाषा",
  "settings.language.search": "भाषाएँ खोजें…",
  "settings.language.empty": "कोई भाषा नहीं मिली।",
  "settings.language.translated": "पूरी तरह अनुवादित",
  "settings.language.beta": "लोकेल लागू · पाठ अंग्रेज़ी में",
  "settings.language.current": "वर्तमान भाषा",
  "settings.language.rtl": "दाएं से बाएं",
  "settings.language.note":
    "“लोकेल लागू” के रूप में चिह्नित भाषाएँ आपके क्षेत्र के प्रारूप और पढ़ने की दिशा सेट करती हैं, जबकि हम पूरी साइट का अनुवाद पूरा कर रहे हैं।",
  "settings.preview.title": "लाइव पूर्वावलोकन",
  "settings.preview.sampleDate": "आज की तारीख",
  "settings.preview.sampleNumber": "सुरक्षित पैकेज",
};

const ar: Partial<TranslationKeys> = {
  "nav.howItWorks": "كيف يعمل",
  "nav.features": "الميزات",
  "nav.porchPartners": "جيران موثوقون",
  "nav.pricing": "الأسعار",
  "nav.help": "المساعدة",
  "nav.getAppFree": "احصل على التطبيق مجانًا",
  "nav.navigate": "تصفّح",
  "nav.getAppFreeLong": "احصل على Porchivo مجانًا — iOS و Android",
  "nav.faqPrompt": "لديك سؤال؟ اقرأ الأسئلة الشائعة",
  "footer.product": "المنتج",
  "footer.company": "الشركة",
  "footer.legal": "القانون والثقة",
  "footer.developers": "المطوّرون والذكاء الاصطناعي",
  "footer.settings": "الإعدادات",
  "footer.rights": "جميع الحقوق محفوظة.",
  "footer.privacy": "الخصوصية",
  "footer.terms": "الشروط",
  "footer.iosAndroid": "iOS و Android",
  "footer.freeToDownload": "تنزيل مجاني",
  "settings.title": "الإعدادات",
  "settings.subtitle": "خصّص كيفية عمل موقع Porchivo بالنسبة لك.",
  "settings.language.title": "اللغة",
  "settings.language.description": "اختر لغتك المفضلة. يتم حفظ اختيارك على هذا الجهاز.",
  "settings.language.label": "لغة العرض",
  "settings.language.search": "ابحث عن اللغات…",
  "settings.language.empty": "لم يتم العثور على لغة.",
  "settings.language.translated": "مترجم بالكامل",
  "settings.language.beta": "تم تطبيق المنطقة · النص بالإنجليزية",
  "settings.language.current": "اللغة الحالية",
  "settings.language.rtl": "من اليمين إلى اليسار",
  "settings.language.note":
    "اللغات المميزة بـ «تم تطبيق المنطقة» تضبط تنسيق منطقتك واتجاه القراءة بينما نكمل ترجمة الموقع بالكامل.",
  "settings.preview.title": "معاينة مباشرة",
  "settings.preview.sampleDate": "تاريخ اليوم",
  "settings.preview.sampleNumber": "الطرود المحمية",
};

export const resources = {
  en: { translation: en },
  es: { translation: es },
  fr: { translation: fr },
  de: { translation: de },
  pt: { translation: pt },
  it: { translation: it },
  ru: { translation: ru },
  zh: { translation: zh },
  ja: { translation: ja },
  ko: { translation: ko },
  hi: { translation: hi },
  ar: { translation: ar },
} as const;
