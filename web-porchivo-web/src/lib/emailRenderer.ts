/**
 * Client-side email template renderer — mirrors the Deno Edge Function
 * `supabase/functions/_shared/emailTemplate.ts` so the preview page can
 * render branded emails without calling the authenticated function.
 *
 * Keep this in sync with the Deno version. The HTML output must match
 * byte-for-byte (modulo whitespace) for preview fidelity.
 */
...
export interface EmailPreviewMeta {
  /** When true, adds .porch-preview-dark to <html> so dark mode can be toggled via JS. */
  preview?: boolean;
}
...
export function renderEmail(
  ...
  @media (prefers-color-scheme: dark) {
    ...
    .porch-cta { background-color: ${DARK_ACCENT_HOVER} !important; }
  ...
  const htmlClass = previewMode
    ? ' class="porch-preview-root porch-preview-dark"'
    : "";
...
/** Toggle dark mode in the preview iframe by adding/removing the CSS class on <html>. */
export function togglePreviewDark(iframe: HTMLIFrameElement, dark: boolean): void {
  const doc = iframe.contentDocument;
  if (!doc) return;
  const htmlEl = doc.documentElement;
  if (dark) {
    htmlEl.classList.add("porch-preview-dark");
  } else {
    htmlEl.classList.remove("porch-preview-dark");
  }
}
